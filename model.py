...
def conv_block2(inputs, l2_lambda):
    ...
    conv25 = Conv1D(16, 25, padding='same', activation='relu',
                    kernel_regularizer=l2(l2_lambda))(inputs)
    
    conv100 = Conv1D(8, 100, padding='same', activation='relu',
                     kernel_regularizer=l2(l2_lambda))(inputs)
...
def multimodal_erg_model(input_shape, img_size, num_clases, window_size, num_variables_adicionales,
    ...
    h_feat, w_feat, c_feat = x_img.shape[1], x_img.shape[2], x_img.shape[3]
    num_patches = h_feat * w_feat

    x_img = Reshape((num_patches, c_feat))(x_img)
    ...
    pos_embed = tf.keras.layers.Embedding(
        input_dim=num_patches, output_dim=d_model, name="img_patch_pos_embedding"
    )(patch_positions)
